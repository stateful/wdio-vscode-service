...
export class ScmView extends SideBarView<typeof ScmViewLocators> {
    ...
    async getProvider(title?: string): Promise<ScmProvider | undefined> {
        ...
        const names = await Promise.all(
            providers.map(async (item) => item.getTitle())
        );
    ...
    async getProviders(): Promise<ScmProvider[]> {
        ...
        const sections: ChainablePromiseElement[] = [];
        for (const header of headers) {
            sections.push(await header.$(this.locators.providerRelative));
        }
        ...
        for (const section of sections) {
            providers.push(
                new ScmProvider(
                    this.locatorMap,
                    section as ChainablePromiseElement,
                    this
                )
            );
        }
...
export class ScmProvider extends BasePage<typeof ScmViewLocators> {
    ...
    async getChangeCount(staged = false): Promise<number> {
        const rows = staged ? await this.stagedChanges$$ : await this.changes$$;
...
export class ScmChange extends ElementWithContextMenu<typeof ScmViewLocators> {
    ...
    async getDescription(): Promise<string> {
        ...
        const length = await desc.length;
        if (length < 1) {
            return "";
        }
    ...
    async isExpanded(): Promise<boolean> {
        ...
        const length = await twisties.length;
        if (length < 1) {
            return true;
        }
        return (
            (await twisties[0].getAttribute("class")).indexOf("collapsed") < 0
        );
    }
...
export class MoreAction extends ElementWithContextMenu<typeof ScmViewLocators> {
    ...
    constructor(locators: VSCodeLocatorMap, public scm: ScmProvider | ScmView) {
    ...
    async openContextMenu(): Promise<ContextMenu> {
        ...
        const length = await shadowRootHost.length;
        if (length > 0) {

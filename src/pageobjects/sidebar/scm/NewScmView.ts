...
export class NewScmView extends ScmView {
    async getProviders(): Promise<ScmProvider[]> {
        ...
        const inputLength = await inputs.length;
        if (inputLength < 1) {
            return [];
        }

        const providers = await this.multiScmProvider$$;
        const providerLength = await providers.length;
        if (inputLength === 1 && providerLength < 1) {
            return [
                await new SingleScmProvider(
                    this.locatorMap,
                    this.singleScmProvider$,
                    this
                ).wait(),
            ];
        }
        ...
        const result: ScmProvider[] = [];
        for (const element of elements) {
            result.push(
                await new MultiScmProvider(
                    this.locatorMap,
                    element as ChainablePromiseElement,
                    this
                ).wait()
            );
        }
...
export interface SingleScmProvider
    extends IPageDecorator<typeof ScmViewLocators> {}
/**
...
export class SingleScmProvider extends ScmProvider {
    ...
    async takeAction(title: string): Promise<boolean> {
        ...
        const names = await Promise.all(
            buttons.map((button) => button.getTitle())
        );
...
export interface MultiScmProvider
    extends IPageDecorator<typeof ScmViewLocators> {}
/**
...
export class MultiScmProvider extends ScmProvider {
    async takeAction(title: string): Promise<boolean> {
        ...
        for (const action of actions) {
            titles.push(await action.getAttribute("title"));
        }
    ...
    async getChanges(staged = false): Promise<ScmChange[]> {
        ...
        if (count > 0) {
            ...
            for (const item of items) {
                ...
                if (
                    indexInner > startIndex &&
                    indexInner <= startIndex + count
                ) {
                    elements.push(item);
                }
            }
    ...
    async getChangeCount(staged = false): Promise<number> {
        const locator = staged
            ? this.locators.stagedChanges
            : this.locators.changes;
...
class MultiMoreAction extends ElementWithContextMenu<typeof ScmViewLocators> {
    ...
    constructor(locators: VSCodeLocatorMap, public scm: ScmProvider) {

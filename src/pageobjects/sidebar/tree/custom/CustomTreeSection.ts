...
export interface CustomTreeSection
    extends IPageDecorator<AllViewSectionLocators> {}
/**
    ...
    async findItem(label: string, maxLevel = 0): Promise<TreeItem | undefined> {
        ...
        for (const element of elements) {
            ...
            if ((await temp.length) > 0) {
                const level = +(await temp[0].getAttribute(
                    this.locatorMap.ViewSection.level as string
                ));
                if (maxLevel < 1 || level <= maxLevel) {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                    item = await new CustomTreeItem(
                        this.locatorMap,
                        element as any,
                        this
                    ).wait();
